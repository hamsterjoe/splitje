...
describe("allocateAdjustmentByPolicy", () => {
    ...
    it("allocates a complete adjustment using percentages", () => {
        const result = allocateAdjustmentByPolicy(
            -101,
            {
                method: "percentage",
        ...
        expect(result).toEqual({
            method: "percentage",
            adjustmentAmountSen: -101,
            allocatedTotalSen: -101,
            remainingSen: 0,
            state: "fully_assigned",
            allocations: [
                {
    ...
    it("allows a percentage adjustment to remain partial", () => {
        const result = allocateAdjustmentByPolicy(
            -101,
            {
                method: "percentage",
        ...
        expect(result.allocatedTotalSen).toBe(-75);
        expect(result.remainingSen).toBe(-26);
        expect(result.state).toBe("partially_assigned");
    });

    it("normalises zero percentage allocations", () => {
        const result = allocateAdjustmentByPolicy(
            0,
            {
                method: "percentage",
        ...
        expect(
            result.allocations.some(({ amountSen }) =>
                Object.is(amountSen, -0),
            ),
        ).toBe(false);
    });

    it("rejects adjustment percentages exceeding 100%", () => {
        expect(() =>
            allocateAdjustmentByPolicy(
                1_000,
                {
